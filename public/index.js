...
function renderTable(prices, currency, shownCount = 5, userPrice = null, searchTerm = '') {
    ...
    // Modal for Ad (replace PayPal)
    html += `
    <div id="ad-modal" style="display:none;position:fixed;top:0;left:0;width:100vw;height:100vh;background:rgba(229,57,53,0.12);z-index:1000;justify-content:center;align-items:center;">
    ...
    if (typeof window.adModalClosed === 'undefined') window.adModalClosed = false;
    ...
    document.addEventListener('click', function (event) {
        ...
        if (adModal && adModal.style.display !== 'none') {
            if (event.target && event.target.id === 'close-ad-modal') {
                adModal.style.display = 'none';
                window.adModalClosed = true;
            }
            if (event.target && event.target.id === 'ad-modal') {
                adModal.style.display = 'none';
                window.adModalClosed = true;
            }
        }
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    form.addEventListener('submit', function (event) {
        ...
        setTimeout(() => {
            ...
            // Show ad modal after calculation
            if (!window.adModalClosed) {
                const adModal = document.getElementById('ad-modal');
                if (adModal) {
                    adModal.style.display = 'flex';
                    adModal.style.justifyContent = 'center';
                    adModal.style.alignItems = 'center';
                    // Optionally inject ad script dynamically
                    // Example:
                    // let script = document.createElement('script');
                    // script.src = 'https://your-ad-network.com/ad.js';
                    // document.getElementById('ad-content').appendChild(script);
                }
            }
        }, 400);
    });
